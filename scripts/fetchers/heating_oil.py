...
def _extract_oil_notations(html: str) -> Dict[str, Optional[float]]:
    ...
    pairs = [
        ('brent', [r'Brent\s*Roh[öo]l', r'Roh[öo]l\s*Brent', r'Brent\s*Crude', r'\bBrent\b'],
                   r'\$\s*/\s*bbl', 'usd_bbl'),
        ('wti',   [r'WTI\s*Crude\s*Oil', r'WTI\s*Crude', r'\bWTI\b', r'West\s*Texas'],
                   r'\$\s*/\s*bbl', 'usd_bbl'),
        ('opec',  [r'Opec[\s\-]*Basket', r'OPEC[\s\-]*Basket', r'\bOpec\b'],
                   r'\$\s*/\s*bbl', 'usd_bbl'),
        ('gasoil',[r'Gas[öo]l', r'Gasöl', r'Gasoil'],
                   r'€\s*/\s*Tonne', 'eur_t'),
    ]
    for key, name_pats, unit_pat, unit_suffix in pairs:
        for name_pat in name_pats:
            pat = (name_pat + r'[^0-9$€]{0,300}?([0-9]+(?:[,.][0-9]+)?)\s*' + unit_pat
                   + r'(?:\s*\(([+\-−][0-9,.\s]+)\))?')
            m = re.search(pat, html, re.DOTALL | re.IGNORECASE)
            if m:
                val = _parse_de_decimal(m.group(1))
                if val and val > 0:
                    out[f'{key}_{unit_suffix}'] = val
